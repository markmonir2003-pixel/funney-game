...
import { Trophy, RefreshCcw, Home, Star, Zap, Skull, Award } from "lucide-react";
...
export function ResultsCard({
  ...
  const message = isDefeated ? "The Shadow caught you! Don't let it win next time!" : getMotivationalMessage(accuracy);
      ...
      <div className={`bg-[#0f172a]/80 backdrop-blur-2xl border ${isDefeated ? 'border-red-500/50' : 'border-white/10'} rounded-[3rem] p-10 text-center shadow-2xl relative overflow-hidden`}>
        ...
        <div className="relative z-10 space-y-2 mb-10">
          <h2 className={`text-4xl font-black tracking-tight leading-tight ${isDefeated ? 'text-red-500' : 'text-white'}`}>
            {isDefeated ? "YOU WERE DEFEATED!" : isPassing ? "Mission Accomplished!" : "Challenge Complete!"}
          </h2>
        ...
        <div className="grid grid-cols-2 gap-4 mb-10">
           ...
           <div className={`bg-cyan-500/10 border border-cyan-500/20 rounded-3xl p-6`}>
              <p className="text-xs font-black uppercase tracking-widest text-cyan-500 mb-1">XP Earned</p>
        ...
        <AnimatePresence>
          {newAchievements.length > 0 && (
            ...
            >
              ...
              <span className="text-yellow-600 font-bold">New Achievement: {newAchievements[0]}!</span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="flex flex-col sm:flex-row gap-4 relative z-10">
          <Button
            onClick={onRetry}
            variant="outline"
            className={`flex-1 h-16 rounded-2xl border-white/10 hover:bg-white/5 text-white font-black text-lg transition-all ${isDefeated ? 'hover:bg-red-500/10 border-red-500/20' : ''}`}
          >
          ...
          <Button
            ...
            className={`flex-1 h-16 rounded-2xl bg-gradient-to-r ${isDefeated ? 'from-red-600 to-red-800 shadow-red-500/20' : 'from-cyan-500 to-blue-600 shadow-cyan-500/20'} hover:opacity-90 text-white font-black text-lg shadow-xl transition-all`}
          >
